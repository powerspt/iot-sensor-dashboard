...
| `example_05_test.ino` | 심화 · 로드셀(HX711) **통신 없이** 테스트 & 보정 (시리얼만) |
| `example_05.ino` | 심화 · 로드셀(HX711) → 서버로 무게 전송 (가이드: `심화_로드셀_HX711.html`) |
| `example_01a.py` | 교재 Python 실습코드 #01 · 시리얼 값 읽기 (짝: example_01.ino) |
| `example_01b.py` | 교재 Python 모델링 예제 #1 · 유선 데이터 로거(CSV·그래프) (짝: example_01.ino) |
| `example_04a.py` | 교재 Python 모델링 예제 #2 · WiFi 소켓 값 받기 (짝: example_04.ino) |
| `example_04b.py` | 교재 Python 모델링 예제 #3 · WiFi 수집→CSV→그래프 (짝: example_04.ino) |
...
- **회원 목록 · 수집 현황** — 전체 회원의 샘플 수·최근값·최근 시각, 회원별 **📈 데이터 보기**(그 회원이 수집한 값을 대시보드로 열람) · **비밀번호 초기화**(임시 비번 발급) · **계정 삭제**(데이터 포함).
- **데이터 보기**는 `/admin/view/<회원id>` — 대시보드와 동일한 조회(센서·기간·집계·임계·CSV)를 그 회원 데이터로 사용합니다. 조회 API는 관리자만 `?uid=`로 대상 지정이 가능하고, **일반 회원이 `uid`를 붙여도 자기 데이터만** 반환됩니다.
