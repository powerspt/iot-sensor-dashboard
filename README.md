...
```
관리자로 로그인하면 헤더의 **관리자** → `/admin`에서:
- **승인 대기** — 신청자(이름·소속·이메일) **승인 / 거절**. 승인 시 API 키가 자동 발급됩니다.
- **회원 직접 생성** — 관리자가 계정을 즉시 만들기(바로 승인).
- **회원 목록 · 수집 현황** — 전체 회원의 샘플 수·최근값·최근 시각.

> 일반 회원은 `/admin`에 접근할 수 없고(자기 데이터만), 승인 전에는 로그인이 막힙니다.
> 기존 DB는 자동 마이그레이션되어(이름/소속/이메일/status 컬럼 추가) 기존 사용자는 `approved`로 유지됩니다.
